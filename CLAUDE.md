...
**Package-specific:**
- `npx lerna run compile --scope @opraiz/package-name` - Build specific package
- `npx lerna run test --scope @opraiz/package-name` - Test specific package
- `npx lerna run watch --scope @opraiz/package-name --include-filtered-dependencies --parallel` - Watch package with dependencies

**Running a single test file (after compile):**
- `npx mocha ./packages/core/lib/browser/some-file.spec.js`

**Test infrastructure:** Tests use Mocha + NYC (Istanbul) for coverage. Config at `configs/mocharc.yml` and `configs/nyc.json`. Each package's `npm test` runs via the `opraizext test` wrapper.
...
- [doc/code-organization.md](doc/code-organization.md) — platform import rules.
- [doc/vscode-usage.md](doc/vscode-usage.md) — rules for exposing Monaco/VS Code internals.
...
- Use `bindRootContributionProvider` (not `bindContributionProvider`) when binding contribution providers in top-level modules.
...
- Use Conventional Commits subjects, matching existing history: `type(scope): summary`.
- Keep commit messages, PR descriptions, and issues **brief and concise**.
- **Security:** never disclose vulnerabilities via a GitHub issue or PR.
